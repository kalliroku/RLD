...
### 1.1 컨셉
**알고리즘이 곧 캐릭터인 시드 기반 로그라이크.** 매 런 다른 모디파이어 아래, 한정된 세르파 풀 (15종 RL 알고리즘) 로 던전을 답파한다. RL 교육은 부산물 — 학명·논문은 hover/툴팁, 표면은 성격 태그.

### 1.2 핵심 가치
- 매 런 다른 시드 + 모디파이어로 변주 (로그라이크의 핵심)
- 알고리즘마다 다른 *성격* — 한정된 세르파 풀에서 라인업을 짜 던전에 던지는 결정 게임
- Q-value · 정책 화살표 · sparkline 으로 학습이 *눈에 보이는* 시각 시그니처 (RL 양념)

### 1.3 타겟 유저
- 짧은 데일리 챌린지 (Wordle 식 10분) + 30분 캠페인 런 둘 다 좋아하는 로그라이크 플레이어
- "AI 가 학습하는 모습" 자체에 호기심을 느끼는 사람 — RL 교육 목적은 아님
- Slay the Spire / Into the Breach / Loop Hero 처럼 결정 위주 게임 팬
...
### 2.2 던전 마스터 모드 (Act 2 — 출시 후 콘텐츠)

> **출시 1.0 스코프 외.** V1.0 캠페인 + 데일리 시드 정착 후 콘텐츠 업데이트로 추가 예정.
> 던전 에디터 (§11) 자체는 이미 구현 완료된 자산이지만, 메인 정체성 카피에서는 분리 (D-4).

1. 그리드 월드 에디터로 던전 설계
2. 타일 배치: 벽, 함정, 몬스터, 보물 등
3. 던전 검증 (BFS + AI 클리어 가능 여부 확인)
4. 던전 공유 → 다른 유저의 도전 (백엔드 도입 시점 미정)
...
### 11.1 개요
브라우저 내에서 타일을 배치해 커스텀 던전을 만들고, AI 를 훈련시켜 볼 수 있는 에디터. Phase 10 구현 완료 자산이며, GDD §2.2 던전 마스터 모드 (Act 2 — 출시 후 콘텐츠) 의 기반이 된다. 메인 정체성 카피에서는 분리 (D-4).
