...
- **D-2026-05-21-1** ✅ **BGM 4트랙 — Web Audio raw 절차적 합성** — VISION §3 M5 1.0 게이트 충족 ([O2] closure). 라이브러리: Web Audio raw (Tone.js 미채택 — 번들 +70KB / 정체성 [[feedback_design_tooling_preference]] Claude 자체 우선 / sound.js 와 톤 정합). 4트랙: T1 Ch.1 (1~3관, A minor, 90~100 BPM, ~45초 루프) / T2 Ch.2 (4~5관, D minor, 80~90 BPM, ~60초) / T3 Ch.3 (6~7관, E phrygian, 100~110 BPM, ~75초) / T4 데일리 (시드 기반 절차적, ~30초 무한). 톤 가이드: STORY 박힘 ("차분 + 그늘, 트라우마 + 미스터리" / 모험가 fanfare 회피 — 모험가 = 적) + D-2026-05-15-15 Crisp/Functional 영문 톤. 전투 stinger / 진엔딩 (W7.5) 별 트랙은 post-1.0. 산업 표준: Cogmind 절차적 / Slay the Spire 4 작곡 / Spelunky chiptune 챕터별 / Hades 작곡 / Risk of Rain 2 작곡+layer — **RLD 이탈점 = 절차적 합성** (정체성 우선의 의도된 이탈). 페이드: chapter 전환 시 cross-fade ~1.5초. mute UI: sound 토글 옆 BGM 토글 추가, i18n 양사전 키 박힘 (D-2026-05-15-16 정합). **검증**: sim 회귀 영향 0 (web/ only), 사용자 시청 검증 필수 (작곡 품질 ear-test 가 grep/build 로 안 됨).

### 2026-06-02

- **D-2026-06-02-18** ✅ **세르파 무한 부활 + G=0 파산 엔딩 — 부활 4충전 한도(D-19-1 #5 / B-203) supersede** — 사용자 결정 (길드 온보딩 설계 중 핵심 루프 재정의). RLD 정체성 [D-2026-05-12-4] "무한 도전으로 답파"(= RL 무한 에피소드 반복)를 서사로 녹임.
  1. **세르파(동료) = 무한 부활.** 던전에서 죽어도 **G(골드) 소모**하고 무제한 재도전. 횟수 한도 없음. (기존 "누적 사망 4/4 = 부활 아이템 충전 한도" 🔁 폐기 — 아래 supersede.)
  2. **G = 0 → 파산 엔딩** (= 캠페인 리셋). 세르파 사망으로 G 고갈되면 즉시 파산. **데스스파이럴(식량부족→출격불가→길드장 직접출정→사망→리셋) 루트는 "질질 끌림"으로 기각**, G=0 단일 트리거로 단순화.
  3. **길드장(플레이어) = 직접 출정 실패 시 무조건 처음부터** (캠페인 리셋). 길드장 직접 진입(manual play, 세르파 0명 온보딩 등)은 부활 대상 아님 — 부활은 *세르파 동료* 전용.
  4. **G 소모 경로**: ① 세르파 부활(상시) ② **모험가 길드 상납금** — 중반부터 도입, 난이도 조절 레버. 흑막(모험가 길드)의 압박·갈취를 게임 시스템화 (STORY 모험가 길드 정합).
  - **Supersedes**: D-2026-05-19-1 #5 ("부활 아이템 = 누적 사망 4/4 충전 한도") 🔁. 코드 `run-state.js:36 DEATH_LIMIT=4` + `deathCount>=DEATH_LIMIT→리셋` (B-203) 은 **재설계 대상** (아직 미구현 — 무한부활 + G소모 + G=0파산 + 길드장 직접실패 트리거로 교체 필요). /commit Critical 게이트의 "누적 사망 4/4" 도 동반 재정의.
  - **미정 (밸런싱 시 확정)**: 세르파 부활 G 단가 / 상납금 액수·주기·도입 챕터 / 식량의 역할 (출정 비용? G와의 관계). STORY.md 미정영역에 동반 기재.
  - **검증**: 설계 박제 단계. 구현 시 캠페인 발란스(25.5/27 ± 2) 재측정 필요.
  - **구현 상태 (2026-06-05)**: **1단계 = 하드 사망 4회 한도/리스타트 분기 제거** 완료 (commit 9142d10 — `DEATH_LIMIT`/`resetForDeathLimit`/B-206 마이그레이션/death_limit i18n 키 폐기, deathCount 는 엔딩 기록 카운터로만 유지). **미구현 후속**: ① 부활 G 소모 ② G=0 파산 엔딩(= 캠페인 리셋 단일 트리거) ③ 길드장 직접출정 실패 리셋 ④ 상납금. 현재는 사망 시 일반 새 런(진행도 리셋)이라 "세르파 부활 시 진행 보존"은 미반영 — G-경제 재설계 시 함께 구현 + 발란스 재측정.
  - **무해 잔존**: 구 localStorage 키 `rld_death_limit_notified` 는 B-206 토스트 제거 후에도 일부 세이브에 남으나 이제 아무도 읽지 않음 — cleanup 코드 추가 대신 방치(추가 dead code 회피).

### 2026-06-15

- **D-2026-06-15-1** ✅ **오프닝 = 정식 플레이 시스템 — SceneRenderer 채택 + RendererRouter 화면+단일스테이지 라우팅 + 시야/길찾기 모델** — 사용자 결정 (인게임 던전 비주얼 작업 중 핵심 전환). 인게임을 오프닝에 맞춰 패치하는 게 아니라, 오프닝(opening-art)의 플레이/렌더 시스템을 *정식 시스템*으로 삼고 부족한 것을 더해 인게임을 완성한다 ([[project_rld_opening_is_play_system]]). 구현 = commit **2091c89** `[reviewed]` (korean-git-reviewer Critical 0).
  1. **(a) opening-art = 인게임 렌더 SSOT.** 단일스테이지 *수동 play* 의 그리기를 `scene-renderer.js`(opening-art 의 PAL/flecks/drawFog/drawTorch/drawCharacter/mulberry32 포팅)가 담당. 오프닝이 "화면을 꽉 채우는" 비결 = 큰 맵이 아니라 **그리드 밖까지 던전 벽으로 메우고 + 플레이어 반경 밖을 분위기 포그로 녹임** → 작은 맵(5×5)도 박스가 아니라 "어두운 던전 속 횃불 챔버"로 읽힌다. **`dungeon-art.js` 은퇴 예정** (TilemapRenderer 의존 확인 후 — 후속). 오브젝트(골드/함정/몬스터/힐/구덩이/보물 gem)는 기존 tile-atlas 이식이 아니라 opening-idiom 으로 신규 작성, `grid.tiles` 직독으로 수집/처치 상태 자동 반영.
  2. **(b) RendererRouter = 화면+단일스테이지 라우팅.** SceneRenderer 가 TilemapRenderer 를 drop-in 대체하는 게 아니라, `renderer-router.js` 가 단일 `this.renderer` 표면 뒤에서 상황별 라우팅: `screen-play && !멀티스테이지 && !training` ⟺ SceneRenderer, 그 외(dev 워크벤치/editor/멀티스테이지/training)는 기존 TilemapRenderer 유지. **라우팅 신호 = `setScene`(syncCamera main.js:5574 + 화면전환 훅 355) — Q-viz 체크박스/follow/showQ/showPolicy 와 분리** (dev 기본 ON 인 Q-viz 에 묶여 빌트인 play 가 follow=false 였던 걸 떼어낸 게 핵심). 두 렌더러가 같은 `#game-canvas` 공유 → scene 활성 시 `_syncTile` 이 TilemapRenderer follow 카메라(자체 RAF)를 꺼서 캔버스 다툼 방지, SceneRenderer.render() 진입 시 480×270 재확보. **점진 교체이자 되돌리기 쉬움** (router 제거 + 생성자 1줄 환원이면 원복). main.js/editor.js 호출부 무변경.
  3. **(c) 시야 = 횃불 반경만 / 길찾기 = 미니맵 분리.** 메인뷰는 "횃불 반경만"(분위기 포그)으로 오프닝 챔버 톤 보존 — 탐색-메모리 딤은 메인뷰에 넣지 않음. **지나온 길/탐색 영역은 우상단 미니맵 trail 이 담당**(main.js `_renderMinimap`, visited Set, screen-play 단일스테이지 전용, 모바일 64px 컴팩트). 오브젝트는 발견형: 포그에 가려졌다가 다가가면 횃불에 드러남. **보물 gem 도 발견형**(카메라 변환 안 + 포그 이전 draw) — 골 비콘 같은 "멀리서 보이는 랜드마크" 아님 (bm 결정 2026-06-21).
  - **[D-2026-05-12-4] 정체성 정합**: ✅ 화면채움 = 그리드 밖 벽 + 포그 연출이지 큰 맵이 아님 → **던전 크기·PPO 학습분포·캠페인 발란스(25.5/27) 불변**. ✅ "RL 교육은 부산물" — RL Q/정책 오버레이는 dev/training(TilemapRenderer) 경로 유지, 클린 play 엔 안 뜸.
  - **결정론**: SceneRenderer = coordSeed+mulberry32(좌표 시드) + now-sin 글로우만. `game.render()` 무인자 → now=0 → 이동 사이 횃불 정적(의도). Math.random/Date.now 0. (헤드리스 2x 렌더 동일 확인.) **렌더↔sim 격리** — `sim/simulator.js` 미참조, 렌더러 통째 교체가 sim/PPO/결정론에 안전 ([[project_rld_runstate_sim_coupling]]).
  - **변경 파일**: `web/js/game/renderer-router.js`(NEW), `web/js/game/scene-renderer.js`, `web/js/main.js`(RendererRouter 생성·syncCamera setScene·화면전환 가드·_renderMinimap·reset visited 시드), `web/css/style.css`(play 캔버스 pixelated + 미니맵).
  - **미정 / 후속 (전부 blocker 아님)**: ① 멀티스테이지 SceneRenderer 포팅 여부(현 TilemapRenderer 유지) ② RL Q/정책 오버레이 SceneRenderer 포팅 vs dev=Tilemap 이원 유지 ③ `dungeon-art.js` 은퇴 시점(TilemapRenderer 의존 확인 후).
  - **검증**: 헤드리스 렌더 정합 확인 완료, 실기 도그푸딩(손맛·미니맵 trail 체감)은 [[project_no_playtest_outreach]] 게이트 — 본인 도그푸딩 통과 후 외부.
