    ...
    test(nEpisodes = 100) {
        ...
        for (let i = 0; i < nEpisodes; i++) {
            ...
            while (steps < testMaxSteps) {
                ...
                const intendedPos = agent.getNextPosition(action);
                const intendedKey = `${intendedPos.x},${intendedPos.y}`;
                if (killedMonsters.has(intendedKey)) this.grid.tiles[intendedPos.y][intendedPos.x] = TileType.EMPTY;
                if (collectedGold.has(intendedKey)) this.grid.tiles[intendedPos.y][intendedPos.x] = TileType.EMPTY;
