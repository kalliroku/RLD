...
/**
 * 퀴니(Quinnie / 내부 키 qkun) — Q-Learning 세르파, 플레이어의 첫 동료. (v2)
 * 갈색 강아지 수인 여성(인간형 본체) — 얼굴 옆으로 처지는 늘어진 개 귀(drop ear,
 * 한쪽 끝 접힘 = 덜렁이) + 가늘고 긴 꼬리끝. 동안 + 따뜻한 볼륨.
 * 밝은 덜렁이(덧니 미소·홍조) + 우직한 충성(호박색 눈).
 * 복장 = 수수한 초보 모험가/길드 잡일 신입 — 여행자 튜닉 + 갈색 조끼 +
 * 심부름 파우치 + rust 네커치프(시그니처 색). v1(무도복·뾰족 귀·옆 포니테일)은
 * "다람쥐로 읽힘" 피드백으로 교정 (브리프 rev 2).
 * repli/rika 와 동일 골격의 waist-up 흉상(torso -19..0 · head -33..-21 ·
 * 눈높이 y=-29 · scale 6 · +x lit) — 무대 드롭인 호환. 광원/딤은 호출부 소유.
 * 산출처/디자인 의도: design/quinnie-portrait/ (Claude Design v2 —
...
export function drawQkunPortrait(ctx, cx, baseY, scale = 6) {
    ...
    const FUR = '#8a5a34', FUR_SH = '#5f3d22', FUR_HI = '#b07d4a';      // 갈색 털/머리(귀·꼬리·머리)
    ...
    // ── 꼬리 — 가늘고 긴 꼬리. −x 아래 모서리에서 살짝 휘어 올라옴(북슬 X) ─
    R(-8, -4, 2, 4, FUR);                      // 뿌리
    ...
    // ── 쫑긋 선 삼각 귀(수인) — 머리 위 양 끝에서 위로 뻗음. 얼굴은 인간형, 동물 신호는
    //    귀+꼬리로만. (드롭귀+머즐 = "개"로 읽혀 제거 — bm 2026-06-12) ──
    // 왼 귀(−x, 그림자) — 밑동 넓고 끝으로 좁아지는 짧은 삼각(여우/늑대, 토끼 X)
    R(-5, -37.5, 3, 3, FUR);                   // 밑동(크라운 위에 얹힘, 넓게)
    R(-5.6, -39.5, 2.4, 2.2, FUR);             // 중간
    R(-6, -41, 1.5, 1.8, FUR);                 // 뾰족한 끝(낮게)
    R(-6, -41, 0.8, 5.5, FUR_SH);              // 바깥 그림자 모서리
    R(-4.3, -38.8, 1.1, 3, EAR_IN);            // 귀 안쪽(분홍)
    // 오른 귀(+x, 빛) — 좌우 대칭
    R(2, -37.5, 3, 3, FUR);                    // 밑동
    R(3.2, -39.5, 2.4, 2.2, FUR);              // 중간
    R(4.5, -41, 1.5, 1.8, FUR);                // 뾰족한 끝
    R(5.2, -41, 0.8, 5.5, FUR_HI);             // lit 모서리(+x)
    R(3.2, -38.8, 1.1, 3, EAR_IN);             // 귀 안쪽

    // ── 얼굴 — 호박색 눈(큰 캐치라이트) + 인간형 코 + 옅은 미소(살짝 덧니) + 홍조 ──
    ...
    R(-0.3, -26.4, 0.8, 0.6, SKIN_SH);         // 코(인간형 — 절제된 콧날 음영)
    R(-1.6, -24.2, 3.2, 0.9, LIP);             // 옅은 미소(닫힌 입술선)
    R(-2.0, -24.6, 0.7, 0.8, LIP);             // 좌 입꼬리(살짝 올림)
    R(1.3, -24.6, 0.7, 0.8, LIP);              // 우 입꼬리(살짝 올림)
    R(-0.3, -24.0, 1.0, 0.5, '#fbf6ea');       // 살짝 드러난 이(작게 — 덧니 느낌만)
