 ...
 * 에크사 - 기대값으로 안정적 학습. 분산 없는 업데이트의 달인.
 *
 ...
 * Q-Learning: Q(s,a) += α[r + γ·max Q(s',a') - Q(s,a)]       (off-policy, uses max)
 * SARSA:      Q(s,a) += α[r + γ·Q(s',a'_next) - Q(s,a)]      (on-policy, uses actual next action)
 * Exp SARSA:  Q(s,a) += α[r + γ·Σ_a π(a|s')·Q(s',a) - Q(s,a)] (uses expected value under policy)
 *
 * Expected SARSA subsumes both: with greedy target policy it equals Q-Learning,
 * with ε-greedy target policy it gives zero-variance updates.
 */

import { Agent, Action, randomAction } from './agent.js';
import { TileType } from './tiles.js';

export class ExpectedSarsa {
    ...
    /**
     * Expected SARSA: compute expected Q-value under ε-greedy policy
     * E[Q(s',a')] = Σ_a π(a|s') · Q(s',a)
     * where π(a|s') = ε/|A| for non-greedy, (1-ε) + ε/|A| for greedy action
     */
    getExpectedQValue(x, y, hp = 100) {
        ...
        for (let a = 0; a < numActions; a++) {
            // ε-greedy probability for this action
    ...
    /**
     * Expected SARSA update: uses expected value under policy (not actual or max)
     */
    learn(state, action, reward, nextState, done) {
        ...
        // Expected SARSA: Σ_a π(a|s') · Q(s',a) instead of max or sampled
        const nextQ = done ? 0 : this.getExpectedQValue(nx, ny, nhp);
