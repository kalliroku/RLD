---
name: handoff-load
description: .handoff/<slug>.md 를 읽고 직전 세션 컨텍스트로 작업 재개. slug 생략 시 목록 + status 표시. PM 핸드오프 (docs/PM/handoffs/) 와는 별개 — 이건 *세션 단위* 스크래치 로딩.
...
- Working dir: !`pwd`
- Available handoffs: !`ls -la .handoff/*.md 2>/dev/null || echo '(empty)'`
- Git status: !`git status --short`

## Task

직전 세션의 `.handoff/<slug>.md` 를 로드해서 작업을 즉시 재개. 컨텍스트 가져온 후 *현재 상태와의 차이* 를 점검해서 stale invariant 가 있으면 사용자에게 알림.

### Step 1: slug 해소

**`$ARGUMENTS` 에 slug 있을 때**:
- `.handoff/<slug>.md` 가 존재하면 Step 2 진행
- 없으면 목록에서 가장 가까운 slug 추측해서 제안 (AskUserQuestion)

**`$ARGUMENTS` 없을 때**:
- `.handoff/*.md` 목록을 status / created / session_summary 와 함께 표시:
  ```
...
`Read` 로 `.handoff/<slug>.md` 전체 읽기. 프론트매터 + 본문 6개 섹션 모두 파싱:
